...
def trilaterate(r_a: float, r_b: float, baseline: float) -> tuple[float, float] | None:
    """2定点 A=(0,0), B=(baseline,0) からの距離 r_a, r_b だけから点の (X,Y) を求める。

    【なぜこれが要るか】
    メジャーで現地で測れるのは2点間の「距離」であって「X方向・Y方向」ではない。
    地上座標系は自分で定義してよい（マーカー1原点、1→2をX軸）が、それを現地作業に
    落とし込むには「距離2本→座標」の変換が要る。A, B を固定アンカーにして
    そこからの距離2本だけ測れば、方向を意識せず全点の座標が一意に求まる。

    【鏡映（裏返し）は距離だけでは区別できない】
    Y>=0 の解を採用する。マーカー・評価対象は全てアンカーを結ぶ直線の同じ側
    （カメラから見える側）に置く前提。

    【アンカー間隔は広く取ること】
    アンカー間隔が測る対象までの距離に対して狭いと、メジャーの誤差が座標に大きく
    増幅される（三角測量の基線長問題と同じ）。実測: 2cm のメジャー誤差で、
    基線 6m だと 50m 先の点の誤差は中央値0.17m/最大0.96m まで悪化するが、
    基線 16m（マーカーの横広がり±8mをそのまま使う）なら中央値0.06m/最大0.42m、
    基線 30m なら中央値0.04m/最大0.21m に収まる（乱数検証済み）。
    **アンカーにはできるだけ離れた2点（理想は10m以上）を選ぶこと。**

    A, B からの距離が三角不等式を満たさない（測定ミス）場合は None を返す"""
    x = (r_a**2 - r_b**2 + baseline**2) / (2 * baseline)
    y2 = r_a**2 - x**2
    if y2 < 0:
        return None
    return float(x), float(np.sqrt(y2))
