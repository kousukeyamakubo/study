"""
損失関数ハイパーパラメータ sweep（narrow angle 版）

FOCAL_GAMMA x FOCAL_ALPHA_pos の 3x3 = 9 通りを探索し、
Stage 4 nuScenes-style mAP（D_TOL=0, A_TOL∈{0,1}, R_TOL=0..5）を評価する。

既存の best_detector_narrow_angle.pt は一切上書きしない。
各ランのモデルは ./sweep_models/ に一時保存する。
"""
...
OUTPUT_DIR   = os.path.join(SCRIPT_DIR, "loss_sweep_narrow_results")
...
# ===== 固定設定 =====
N_FIXED      = 10
FIXED_ANGLES = np.linspace(1, 4, N_FIXED)   # narrow 角度グリッド [度]
...
# ===== Stage 4 評価設定（tolerance_sweep_narrow.py と同一） =====
A_TOLS = [0, 1]
R_TOLS = list(range(6))   # 0..5
D_TOL  = 0

print(f"DEVICE: {DEVICE}")
print(f"sweep grid: GAMMA={GAMMA_VALUES}, ALPHA_pos={ALPHA_POS_VALUES}")
print(f"total runs: {len(GAMMA_VALUES) * len(ALPHA_POS_VALUES)}")


# ===== モデル定義（check.ipynb / tolerance_sweep_narrow.py と同一） =====
...
# ===== データセット定義（check.ipynb と同一ロジック） =====

class MultiTargetDatasetSeg(Dataset):
    def __init__(self, meta_df, fixed_angles=FIXED_ANGLES):
        self.samples = []
        ...
        for idx in range(len(meta_df)):
            ...
            # 点ラベル: 0=背景 / 1=cy / 2=ve（重なり時は ve 優先）
...
def make_loss_fn(gamma, alpha_pos):
    """alpha: [bg=1.0, cy=alpha_pos, ve=alpha_pos]"""
    ...
    def compute_loss(logits, y_seg):
        ...
        ce  = F.cross_entropy(logits, y_seg, weight=a, reduction="none")  # (B, N_FIXED, H, W)
        p_t = torch.exp(-ce)
...
def train_model(train_loader, val_loader, gamma, alpha_pos, run_name):
    ...
    best_state    = None
    save_path     = os.path.join(MODELS_DIR, f"{run_name}.pt")
...
# ===== mAP 評価ロジック（tolerance_sweep_narrow.py と同一） =====
    ...
    with torch.no_grad():
        for idx in range(len(eval_df)):
            ...
            if vcy:
                ...
                cy_sample_info.append(dict(sample_idx=idx, r_diff=r_diff,
                                           n_det=len(cy_voxels),
            ...
            if vve:
                ...
                ve_sample_info.append(dict(sample_idx=idx, r_diff=r_diff,
                                           n_det=len(ve_voxels),
                                           gt_pred_label=int(pred[tch, tru_d, tru_r].item())))
            else:
                tru = dict(valid=False, true_ch=0, true_d=0, true_r=0,
...
def compute_ap(raw, n_gt, a_tol, d_tol, r_tol):
    """greedy matching による AP（PASCAL VOC 方式）"""
    dets = []
    for rec in raw:
        if rec["valid"]:
...
def compute_stage4_map(model, eval_df):
    """Stage 4 nuScenes-style mAP を返す"""
...
eval_df   = fixed_df.iloc[200:].reset_index(drop=True)   # holdout 100件
...
# ===== ヒートマップ出力（GAMMA x ALPHA_pos の mAP/AP_cy/AP_ve） =====

fig, axes = plt.subplots(1, 3, figsize=(15, 4))
gammas     = sorted(results_df["gamma"].unique())
alpha_vals = sorted(results_df["alpha_pos"].unique())
...
for ax, metric, title in [
    ...
    grid = np.zeros((len(gammas), len(alpha_vals)))
    for i, g in enumerate(gammas):
        for j, a in enumerate(alpha_vals):
            row = results_df[(results_df["gamma"] == g) & (results_df["alpha_pos"] == a)]
            grid[i, j] = float(row[metric].values[0]) if len(row) else 0.0
    ...
    for i in range(len(gammas)):
        for j in range(len(alpha_vals)):
            ax.text(j, i, f"{grid[i,j]:.3f}", ha="center", va="center",
                    color="white" if grid[i,j] < 0.6 else "black", fontsize=11)
    plt.colorbar(im, ax=ax)

fig.suptitle(
    "Loss Hyperparameter Sweep — narrow angle\n"
    "Stage 4 nuScenes-style mAP (D_TOL=0, A_TOL∈{0,1}, R_TOL=0..5)",
    fontsize=12,
)
plt.tight_layout()
...
# ===== best 設定の表示 =====
best_row = results_df.loc[results_df["mAP"].idxmax()]
print(f"\n=== Best 設定 ===")
...
print(f"  現状の設定（gamma=2.0, alpha=500）との比較:")
baseline = results_df[(results_df["gamma"] == 2.0) & (results_df["alpha_pos"] == 500.0)]
if len(baseline):
    b = baseline.iloc[0]
    print(f"    baseline mAP = {b['mAP']:.4f}  (AP_cy={b['AP_cy']:.4f}, AP_ve={b['AP_ve']:.4f})")
