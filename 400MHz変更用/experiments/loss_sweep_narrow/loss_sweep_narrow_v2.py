"""
loss_sweep_narrow v2

v1 から拡張:
  GAMMA : [3.0, 4.0, 5.0]
  ALPHA : [400, 500, 600]

gamma=3.0, alpha=500 は v1 済みのためスキップし、v1 結果を流用して
3×3 ヒートマップを生成する。
"""
...
OUTPUT_DIR  = os.path.join(SCRIPT_DIR, "loss_sweep_narrow_v2_results")
MODELS_DIR  = os.path.join(SCRIPT_DIR, "sweep_models")   # v1 と共有
V1_CSV      = os.path.join(SCRIPT_DIR, "loss_sweep_narrow_results", "sweep_results.csv")
...
ALPHA_POS_VALUES = [400.0, 500.0, 600.0]
...
new_runs = [(g, a) for g, a in itertools.product(GAMMA_VALUES, ALPHA_POS_VALUES)
            if (g, a) not in V1_DONE]
print(f"新規実行: {len(new_runs)} 通り  スキップ(v1済み): {len(V1_DONE)} 通り")
...
for gamma, alpha_pos in itertools.product(GAMMA_VALUES, ALPHA_POS_VALUES):
    if (gamma, alpha_pos) in V1_DONE:
        print(f"\n--- gamma{gamma}_alpha{int(alpha_pos)}: v1 済みのためスキップ ---")
        continue
...
# v2 グリッドに含まれる v1 結果のみ抽出
v1_subset = v1_df[
    v1_df["gamma"].isin(GAMMA_VALUES) & v1_df["alpha_pos"].isin(ALPHA_POS_VALUES)
].copy()

new_df  = pd.DataFrame(new_results)
combined_df = pd.concat([v1_subset, new_df], ignore_index=True).sort_values(
    ["gamma", "alpha_pos"]
).reset_index(drop=True)

csv_path = os.path.join(OUTPUT_DIR, "sweep_results_v2.csv")
...
for ax, metric, title in [
    ...
    for i, g in enumerate(gammas):
        for j, a in enumerate(alpha_vals):
            ...
            grid[i, j] = float(row[metric].values[0]) if len(row) else float("nan")
...
fig.suptitle(
    "Loss Hyperparameter Sweep v2 — narrow angle\n"
    "Stage 4 nuScenes-style mAP (D_TOL=0, A_TOL∈{0,1}, R_TOL=0..5)",
    fontsize=12,
)
plt.tight_layout()
heatmap_path = os.path.join(OUTPUT_DIR, "sweep_heatmap_v2.png")
plt.savefig(heatmap_path, dpi=150, bbox_inches="tight")
plt.close()
print(f"saved: {heatmap_path}")

# ===== best 設定 =====
best_row = combined_df.loc[combined_df["mAP"].idxmax()]
print(f"\n=== Best 設定（v1+v2 合算） ===")
...
json_path = os.path.join(OUTPUT_DIR, "sweep_results_v2.json")
with open(json_path, "w", encoding="utf-8") as f:
    json.dump({
        "config": {
            "GAMMA_VALUES": GAMMA_VALUES, "ALPHA_POS_VALUES": ALPHA_POS_VALUES,
