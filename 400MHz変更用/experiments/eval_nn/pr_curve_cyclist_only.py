"""
PR曲線・AP評価スクリプト — cyclist-only シナリオ版

評価モデル : best_detector_narrow_angle.pt
評価データ : learn_dataset_narrow_angle_single の cyclist_only holdout (260件のうちシャッフル後 340件目以降)
スコア     : クラス1(cyclist)のsoftmax確率の全ボクセル最大値
...
OUTPUT_DIR     = os.path.join(SCRIPT_DIR, "pr_curve_cyclist_only_results")
...
def build_eval_df():
    """narrow_angle_single の cyclist_only holdout — dataset.md の分割定義に従う"""
    df = pd.read_csv(SINGLE_META_CSV)
    df = df[df["valid_all"] == 1].reset_index(drop=True)
    df = df.sample(frac=1, random_state=RANDOM_SEED).reset_index(drop=True)
    holdout = df.iloc[340:].reset_index(drop=True)
    # cyclist のみが存在するシナリオに絞る
    return holdout[holdout["scenario"] == "cyclist_only"].reset_index(drop=True)
...
def collect_detections(model, eval_df):
    """
    cyclist クラスのみ評価する。
    戻り値: cy_dets (list of {'score', 'is_hit', 'sample_idx'}), n_gt_cy
    """
    cy_dets = []
    n_gt_cy = 0
    ...
    with torch.no_grad():
        for idx in range(len(eval_df)):
            ...
            if valid_cy:
                ...
                # cyclist_only シナリオで valid_cy=0 のサンプルは FP 候補として収集
                for ch, d, r, sc in voxels_for_class(1):
                    cy_dets.append({"score": sc, "is_hit": False, "sample_idx": idx})

    return cy_dets, n_gt_cy
...
print(f"eval samples (cyclist_only): {len(eval_df)}")

print("running inference...")
cy_dets, n_gt_cy = collect_detections(model, eval_df)
print(f"GT: cy={n_gt_cy}")
...
ax.step(recs_cy, precs_cy, where="post", color="tab:blue", lw=2, label=f"AP={ap_cy:.4f}")
ax.fill_between(recs_cy, precs_cy, step="post", alpha=0.15, color="tab:blue")
ax.set_xlabel("Recall")
ax.set_ylabel("Precision")
ax.set_title(
    f"Cyclist  AP={ap_cy:.4f}  (nGT={n_gt_cy})\n"
...
plt.tight_layout()
out_png = os.path.join(OUTPUT_DIR, "pr_curve_cyclist_only.png")
