"""
Tolerance sweep for PR curve evaluation.

Stage 1: diagonal sweep  (A_TOL, D_TOL, R_TOL) = (k, k, k)  for k in 0..DIAG_MAX
Stage 2: per-axis sweep  fix best diagonal tolerance, vary one axis at a time

推論は1回だけ実行してスコア・検出位置をキャッシュし、
許容幅ごとに TP/FP を再判定して AP を計算する。
"""
...
def build_eval_df():
    """fixed_angle holdout (100件) — 2物体シーンのみ（cy+ve 同時存在）"""
...
# Stage 6 で ap_cy_avg 等の同名変数が上書きされるため、サマリ用に別名で保存
...
# プロット: AP vs R_TOL で A_TOL=0 / A_TOL=1 を比較 (cy, ve, mAP の 3 パネル)
...
fig.suptitle(
    f"Stage 4: nuScenes-style mAP  (D_TOL=0, avg over A_TOL×R_TOL)\n"
...
ax_p.grid(alpha=0.3)
ax_p.set_title(
    f"D_TOL=0, A_TOL∈{{0,1}}, R_TOL=0–5  mAP={s4_mAP_avg:.4f}",
    fontsize=FP_TITLE,
)
plt.tight_layout()
plt.savefig(os.path.join(FOR_PAPER_DIR, "nuscenes_map_vs_rtol.png"), dpi=300, bbox_inches="tight")
plt.close()
print("saved (for_paper): nuscenes_map_vs_rtol.png")
...
for (lo, hi), label in zip(RDIFF_BINS, RDIFF_LABELS):
    ...
    for a_tol in [0, 1]:           # Stage 4 と同じ A_TOL ∈ {0, 1}
        for r_tol in R_TOLS_S3:    # 0..5
...
ax.set_ylabel("AP (nuScenes-style, avg A_TOL×R_TOL, D_TOL=0)")
...
ax_p.set_title(
    f"D_TOL=0, A_TOL∈{{0,1}}, R_TOL=0–5  "
    f"AP_cy={float(np.mean([r['AP_cy'] for r in stage6])):.4f}  "
    f"AP_ve={float(np.mean([r['AP_ve'] for r in stage6])):.4f}",
    fontsize=FP_TITLE,
)
...
plt.close()
print("saved (for_paper): stage6_rdiff_ap.png")
...
def failure_category(info, matched_set, cls_idx):
    ...
    if gl == cls_idx:
        return "pos_miss"   # GT voxel correct label but pos off (理論上発生しない)
    if gl == 0:
        return "no_det"     # GT voxel が背景に分類され、他に検出なし or 検出が外れ
    return "gt_confused"    # GT voxel が他クラスに混同（class confusion）
...
for cls_name, sample_info, matched_set, cls_idx in [
    ...
    print(f"    TP            : {n_tp:>3}")
    print(f"    no_det        : {n_nodet:>3}  (GT ボクセルが背景に分類→検出なし or 位置ずれ)")
    print(f"    gt_confused   : {n_confused:>3}  (GT ボクセルが他クラスに混同)")
    print(f"    pos_miss      : {n_pos:>3}  (理論上 0 のはず)")
...
plt.close()
print("saved (for_paper): stage7_failure.png")
...
out_json = os.path.join(OUTPUT_DIR, "tolerance_sweep_results.json")
